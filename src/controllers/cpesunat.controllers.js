...
async function generarPDFTicketEncomienda(req, res) {
  try {
    ...
    const { logo: logoBuffer } = rows[0];
    const resultadoPdf = await cpegenerapdfticketencomienda(logoBuffer, dataTicket);
    ...
    console.error('Error al generar ticket de encomienda:', error);
    return res.status(400).json({
...
const registrarCPETicketEncomiendaPDF = async (req, res, next) => {
