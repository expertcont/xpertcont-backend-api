...
const cpegenerapdf = async (size, logo, jsonVenta, digestvalue) => {
  ...
  const runLayout = (page, draw, pageHeight) => {
    ...
    consumed += 12+5; //ref a linea anterior
    ...
    // ── NEW EXONERADO ────────────────────────────────────────────────────────────
    if (venta.base_exonerada != null && Number(venta.base_exonerada) !== 0) {
      if (draw) {
    ...
    if (draw) {
      ...
      P.drawText(numeral(monto_total).format('0,0.00')?.toString() ?? "", { x:ticketWidth-tw-margin-marginLeftSize, y:Y(), size:fontSize+2, font:fontNegrita });
    }
    consumed += 15;
